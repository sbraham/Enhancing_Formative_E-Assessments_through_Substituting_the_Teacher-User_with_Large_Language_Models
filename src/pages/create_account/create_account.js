//build console.log('Loading: sign-up.js');
...
create_account_form.addEventListener("submit", async (event) => {
    ...
    if (password !== confirm_password) {
        //build console.warn(`create_account_form: Passwords do not match.`);
        alert(`Passwords do not match. Please take care when inputting your password.`);
        return;
    }
    
    //build console.log(`createUser: awaiting response...`);
    let responce = await createUser(email, password);
    //build console.log(`createUser: received`);
...
login_link.addEventListener('click', function(event) {
    //build console.log(`login_link clicked`);
