...
let is_batch = false;
let is_question_evaluation = false;
...
if (is_question_evaluation || is_hallucination_evaluaton) {
    let quiz_1 = []
    let quiz_2 = []
    ...
    for (const context of contexts_for_quiz_questions) {
        quiz_1 = await BatchSWQG(number_of_questions, 'multiple_choice', `${context}`, 4, is_hallucination_evaluaton);
        console.log(quiz_1);
        quiz_2 = await BatchSWQG(number_of_questions, 'multiple_choice', `${context}`, 4, is_hallucination_evaluaton);
        console.log(quiz_2);
